...
def create_driver_function(functions, num_samples_per_func):
    ...
    driver_code.append("")
    driver_code.append("int enzyme_dup;")
    driver_code.append("int enzyme_const;")
    driver_code.append("template <typename return_type, typename... T>")
    driver_code.append("return_type __enzyme_fp_optimize(void *, T...);")

    driver_code.append("")
    driver_code.append("int main(int argc, char* argv[]) {")
    driver_code.append('    std::string output_path = "";')
    driver_code.append("    bool profiling_mode = false;")
    driver_code.append(f"    int num_samples = {num_samples_per_func};")
    driver_code.append("")
    ...
    driver_code.append("            }")
    driver_code.append('        } else if (std::strcmp(argv[i], "--prof") == 0) {')
    driver_code.append("            profiling_mode = true;")
    driver_code.append(f"            num_samples = {max(100, num_samples_per_func // 1000)};  // Use 1/1000th of samples for profiling, minimum 100")
    driver_code.append("        }")
    driver_code.append("    }")
    driver_code.append("")
    driver_code.append("    bool save_outputs = !output_path.empty();")
    driver_code.append("")
    driver_code.append("    std::mt19937 gen(profiling_mode ? 123 : 42);  // Different seed for profiling")
    driver_code.append("")
    ...
    for func_name, bounds, params, return_type in functions:
        driver_code.append(f"    for (int i = 0; i < num_samples; ++i) {{")
        ...
        for param in params:
            ...
            driver_code.append(f"        auto {param_var_name} = ({param_type}){param_value};")
            call_params.append(param_var_name)

        call_params_str = ", ".join(call_params)

        driver_code.append(f"        double res = __enzyme_fp_optimize<{return_type}>((void*){func_name}, {call_params_str});")
